    ...
    createDatabase(excelFileInput,dbFileInput){
        ...
        return new Promise((resolve,reject)=>{
            if (fs.existsSync(settingFile)&&fs.existsSync(databaseFile)) {
                ...
                reject('No Database Setting xlsx and Database Sqlite');
            }
        })
        ...
        return new Promise((resolve,reject)=>{
            ...
            try {
                ...
                xlsxtojson1st({
                    ...
                    let createTablePromise = new Promise((rlve,rjct)=>{
                        
                        if (distinct_table_name.length===0) rlve('No table created');
                        ...
                        distinct_table_name.forEach(el=>{
                            ...
                            if (table){
                                ...
                                .then(data=>{
                                    countIdx++;
                                    if (countIdx>=distinct_table_name.length){
                                        rlve('created '+countIdx+' table(s)');
                                    }
                                })
                                .catch(err=>{
                                    rjct(err);
                    ...
                    .then(result=>{
                        ...
                        console.log(result,distinct_table_name)
                    ...
                    if (sheet!=undefined){
                        ...
                        for (let i=0;i<jsonOut.length;i++){
                            ...
                            .then(data=>{
                                ...
                                countResult++;
                                if (countResult>=jsonOut.length) resolve('Insert data OK!')
                            })
                            .catch(err=>{
                                //console.log(err);
                                reject(err);
